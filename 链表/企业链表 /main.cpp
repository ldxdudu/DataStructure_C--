...
template<> void myPrint(LinkNode *data){
	using namespace std;
	Person *p = reinterpret_cast<Person *> (data); // 在C++语言中，reinterpret_cast主要有三种强制转换用途：改变指针或引用的类型、将指针或引用转换为一个足够长度的整形、将整型转换为指针或引用类型。
	// Person *p = (Person *)data;
	cout << "name:" << p->myName << " age:" << p->myAge << " score:" << p->myScore << endl;
}
// 模板重载，将myCompare函数模板进行重载
template<> bool myCompare(LinkNode *d1, LinkNode *d2) {
	Person *data1 = (Person *) d1;
	Person *data2 = (Person *) d2;
...
int main(){
	...
	list->insert(0, (LinkNode *)&p1);
	list->insert(0, (LinkNode *)&p2);
	list->insert(0, (LinkNode *)&p3);
	list->insert(0, (LinkNode *)&p4);
	list->insert(0, (LinkNode *)&p5);

	list->printLinkList(); // 打印链表

	Person p6("bbb", 22, 100);
	using namespace std;
	cout << "find p6 pos: "<< list->find((LinkNode *)&p6) << endl;
